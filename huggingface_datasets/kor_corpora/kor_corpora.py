...
_KO_QPAIR_SPLIT = {
  ...
  'split': {
    datasets.Split.TRAIN: lambda x: True,
    datasets.Split.VALIDATION: lambda x: True,
    datasets.Split.TEST: lambda x: True,
}}
...
_KO_NLI_SPLIT = {
  'source': {
    datasets.Split.TRAIN: ['mnli_train', 'snli_train'],
    datasets.Split.VALIDATION: ['xnli_dev'],
    datasets.Split.TEST: ['xnli_test'],
  },
  'split': {
    datasets.Split.TRAIN: lambda x: True,
    datasets.Split.VALIDATION: lambda x: True,
...
class KorCorporaConfig(datasets.BuilderConfig):
  def __init__( self,
                name,
                text_features,
                label_column,
...
class KorCorpora(datasets.GeneratorBasedBuilder):
  ...
  def _info(self) -> datasets.DatasetInfo:
    ...
    if self.config.additional_feat is not None:
      features.update(_get_additional_feat_dict(self.config.additional_feat))
  ...
  def _split_generators(self, dl_manager: datasets.DownloadManager):
    ...
    if self.config.manual_split is not None:
      path = _update_split(path, self.config.manual_split)
      split_fn = self.config.manual_split['split']
      #return {k:self._generate_examples(v, split_fn[k]) for k, v in path.items()}
      return [datasets.SplitGenerator(name=k, gen_kwargs={'path_list': v, 'split_fn': split_fn[k]}) for k, v in path.items()]
    ...
    return [datasets.SplitGenerator(name=k, gen_kwargs={'path_list': v}) for k, v in path.items()]

  def _generate_examples(self, path_list, split_fn=None):
    """Yields examples."""
    process_label = self.config.process_label
    label_classes = self.config.label_classes
    additional_feat = self.config.additional_feat
    ...
    for path in path_list:
      with open(path) as f:
        if self.config.name.startswith('qpair'):
          reader = csv.DictReader(f, delimiter="\t", quoting=csv.QUOTE_MINIMAL)
        else:
          reader = csv.DictReader(f, delimiter="\t", quoting=csv.QUOTE_NONE)
        for row in reader:
          example = {
            k: row[col] for k, col in self.config.text_features.items()
          }
          example['idx'] = idx
          idx += 1
